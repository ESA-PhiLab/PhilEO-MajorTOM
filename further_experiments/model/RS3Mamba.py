...
class FusionAttention(nn.Module):
    ...
    def pad(self, x, ps):
        ...
        if W % ps != 0:
            ...
            x = F.pad(x, (0, ps - W % ps)) 
            #x = F.pad(x, (0, ps - W % ps), mode='reflect') 
            #x = nn.ReflectionPad2d((0, ps - W % ps))(x)  
...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        # self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
        #                                   out_indices=(1, 2, 3, 4), pretrained=False, channels=10) 
        
        # self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
        #                                   out_indices=(1, 2, 3, 4), pretrained=False, in_chans=10)
        self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
                                          out_indices=(1, 2, 3, 4), pretrained=True, in_chans=10)
        #self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
        #                                   out_indices=(1, 2, 3, 4), pretrained=True, in_chans=10)     
        ...
        #)            
        self.stem = nn.Sequential(
              nn.Conv2d(10, 48, kernel_size=7, stride=2, padding=3),
              nn.InstanceNorm2d(48, eps=1e-5, affine=True),
        )
        #self.stem2 = nn.Sequential(
        #       nn.Conv2d(10, 3, kernel_size=7, stride=2, padding=3),
        #       nn.InstanceNorm2d(3, eps=1e-5, affine=True),
        # ) 
        ...
        # )     
        # self.stem2 = nn.Sequential(
        #       nn.Conv2d(48, 3, kernel_size=7, stride=2, padding=3),
        #       nn.InstanceNorm2d(3, eps=1e-5, affine=True),
        # )
        ...
        # self.linear_encode = nn.Sequential(
        #     nn.Conv2d(self.linear_dim, self.latent_dim, kernel_size=7, stride=2, padding=3),
        #     nn.InstanceNorm2d(self.latent_dim, eps=1e-5, affine=True),
        # )
        # self.stem = nn.Sequential(
        #       nn.Conv2d(10, 48, kernel_size=7, stride=2, padding=3),
        #       nn.InstanceNorm2d(48, eps=1e-5, affine=True),
    ...
    def forward(self, x):
        ...
        #print(ress[3].reshape(-1, ress[3].shape[1]*ress[3].shape[2]*ress[3].shape[3]).shape) 

        flat = ress[3].reshape(-1, ress[3].shape[1]*ress[3].shape[2]*ress[3].shape[3])
        ...
        # print(out_clouds.shape)
        # print(out_buildings.shape)
        # print(out_landcover.shape)
        ...
        #return x, embeddings, embeddings, embeddings, (out_coords, out_clouds, out_buildings, out_landcover) 

        #reconstruction, embeddings, embeddings_cnn, decoded, predictions 

        #embeddings, embeddings_cnn, skips, (out_coords, out_clouds, out_buildings, out_landcover)             
...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        #                                   out_indices=(1, 2, 3, 4), pretrained=pretrained)    
        self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
                                          out_indices=(1, 2, 3, 4), pretrained=pretrained, in_chans=10)
        self.conv1 = self.backbone.conv1
        self.bn1 = self.backbone.bn1
        self.act1 = self.backbone.act1
        self.maxpool = self.backbone.maxpool
        ...
        #self.vssm_encoder = VSSMEncoder(patch_size=2, in_chans=48)
        self.vssm_encoder = VSSMEncoder(patch_size=2, in_chans=48)
        #self.vssm_encoder = VSSMEncoder(patch_size=4, in_chans=48)       
        encoder_channels = self.backbone.feature_info.channels() 
        ssm_dims=[96, 192, 384, 768]
